...
vr::EVRInitError CSampleControllerDriver::Activate(vr::TrackedDeviceIndex_t unObjectId)
{
	...
	vr::VRProperties()->SetStringProperty(m_ulPropertyContainer, Prop_ControllerType_String, "webhandsvr");

	vr::VRProperties()->SetStringProperty(m_ulPropertyContainer, Prop_ModelNumber_String, "WebHandsVr");
	vr::VRProperties()->SetStringProperty(m_ulPropertyContainer, Prop_ManufacturerName_String, "WebHandsVr");
	...
	vr::VRProperties()->SetStringProperty(m_ulPropertyContainer, Prop_RenderModelName_String, "vive_controller");
	vr::VRProperties()->SetStringProperty(m_ulPropertyContainer, Prop_TrackingSystemName_String, "webhandsvr");
	vr::VRProperties()->SetInt32Property(m_ulPropertyContainer, Prop_DeviceClass_Int32, TrackedDeviceClass_Controller);

	vr::VRProperties()->SetStringProperty(m_ulPropertyContainer, Prop_SerialNumber_String, "WebHandVrLeft" + (char)ControllerIndex);
	...
	vr::VRProperties()->SetStringProperty(m_ulPropertyContainer, Prop_InputProfilePath_String, "{webhandsvr}/input/webhandsvr_profile.json");
	...
	vr::VRDriverInput()->CreateBooleanComponent(m_ulPropertyContainer, "/input/joystick/click", &HButtons[2]);
	...
	vr::VRDriverInput()->CreateScalarComponent(
		m_ulPropertyContainer, "/input/grip/value", &HAnalog[3],
		vr::EVRScalarType::VRScalarType_Absolute, vr::EVRScalarUnits::VRScalarUnits_NormalizedOneSided);
...
std::string CSampleControllerDriver::GetSerialNumber() const
{
	...
	{
	case 1:
		return "WebHandVrLeft";
		break;
	case 2:
		return "WebHandVrRight";
		break;
	}
}
